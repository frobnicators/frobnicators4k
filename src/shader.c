...
void init_shaders() {
	...
	const unsigned long len = strlen(GLSL_VERSION_LINE) + strlen(common.data) + strlen(vert_data.data) + 1;
	char * src = (char*)malloc(len);
	sprintf(src, GLSL_VERSION_LINE "%s%s", common.data, vert_data.data);
...
#if _DEBUG
	struct file_data_t common = read_data("common.glsl");
	char * src = (char*)malloc(strlen(GLSL_VERSION_LINE) + strlen(common.data) + strlen(frag_data.data) + 1);
	sprintf(src, GLSL_VERSION_LINE "%s%s", common.data, frag_data.data);
